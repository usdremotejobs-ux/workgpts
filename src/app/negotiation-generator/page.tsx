...
const BEST_FOR: Record<string, string[]> = {
    empathetic: ["1-on-1 Conversations", "Mentoring & Coaching", "Sensitive Feedback"],
    authority: ["Performance Corrections", "Deadline Escalations", "Repeated Issues"],
    leadership: ["Team Alignment", "Strategic Direction", "Cross-functional Conflicts"],
};
...
export default function NegotiationGenerator() {
    ...
    const { completion, input, handleInputChange, handleSubmit, isLoading } = useCompletion({
        api: "/api/chat",
        streamProtocol: "text",
        body: { tool: "negotiation-generator" },
        onError: (error: Error) => {
    ...
    const empatheticText = extractSection(completion, "## 💬 Empathetic Style", "## 📌 Authority Style");
    const authorityText = extractSection(completion, "## 📌 Authority Style", "## ⚡ Leadership Style");
                ...
                <div className="bg-card rounded-xl border shadow-sm p-5 relative flex flex-col min-h-[340px]">
                    {!hasOutput && !isLoading ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground space-y-4 py-10">
                            ...
                            <p className="text-sm">Your negotiation messages will appear here.</p>
                        </div>
                    ) : (
                        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col flex-1">
                            ...
                            {/* Tab content panels */}
                            {(["empathetic", "authority", "leadership"] as const).map((tab) => (
